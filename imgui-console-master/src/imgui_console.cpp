...
void ImGuiConsole::Draw(bool* b)
{
    ...
    if (!ImGui::Begin("console", b, ImGuiWindowFlags_MenuBar))
    {
        ImGui::PopStyleVar();
        ImGui::End();
        return;
...
void ImGuiConsole::InitIniSettings()
{
    ...
    if (g.Initialized)
    {
        if (!g.SettingsLoaded && !m_LoadedFromIni)
        {
...
void ImGuiConsole::LogWindow()
{
    ...
    if (ImGui::BeginChild("ScrollRegion##", ImVec2(0, -footerHeightToReserve), false, 0))
    {
        ...
        m_ScrollToBottom = false;
        //std::cout << "Error occurred in line " << __LINE__ << std::endl;
