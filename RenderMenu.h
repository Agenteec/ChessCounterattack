...
public:
    ...
    {
        if (ImGui::CollapsingHeader(u8"Клавиатура"))
        {
            #pragma region Прыжок
            ImGui::Text(u8"Прыжок");
            ImGui::SameLine();
            ImGui::PushID("Jump");
    ...
    {
        ImGui::Text(u8"Настройки графики");
        ImGui::SliderFloat(u8"Яркость", &tempSettings.video.brightness, 0.0f, 1.0f);
        ...
        ImGui::Checkbox(u8"Полный экран", &tempSettings.video.FullScreen);
	...
	static int ShovMenu(sf::RenderWindow* window)
	{
        ...
        if (showSettings)
        {
            ...
            ImGui::Begin(u8"Настройки", &showSettings);

            if (ImGui::Button(u8"Графика"))
            {
            ...
            else if (showSoundSettings)
            {
                ...
                ImGui::SliderFloat(u8"Громкость окружения", &tempSettings.volume.ambientVolume, 0.0f, 1.0f);
                ImGui::SliderFloat(u8"Громкость музыки", &tempSettings.volume.musicVolume, 0.0f, 1.0f);
            }
            ...
            if (ImGui::Button(u8"Ок", ImVec2(80, 0))) {
            ...
            if (ImGui::Button(u8"Отмена", ImVec2(80, 0))) {
                // Действия, выполняемые при нажатии на кнопку
            ...
            if (ImGui::Button(u8"Применить", ImVec2(100, 0))) {
                // Действия, выполняемые при нажатии на кнопку
                if (tempSettings.SetSettings(window)|| tempSettings.video.FullScreen != CGlobalSettings.video.FullScreen)
                {
                    showReloadDialog = true;
