...
{
    ImGui::OpenPopup(u8"Перезагрузить приложение?");
    
    // Confirmation
    if (ImGui::BeginPopupModal(u8"Перезагрузить приложение?", &showReloadDialog, ImGuiWindowFlags_AlwaysAutoResize))
    {
        //std::cout << "Worked\n";
        ImGui::Text(u8"Для изменения некоторых параметров нужна перезагрузка,\n перезагрузить?\n\n");
        ImGui::Separator();

        if (ImGui::Button(u8"Перезагрузить", ImVec2(120, 0)))
        {
        ...
        ImGui::SameLine();
        if (ImGui::Button(u8"Нет", ImVec2(120, 0)))
        {
            showReloadDialog = false;
            ImGui::CloseCurrentPopup();
        }
